...
function origenBadgeClass(origen: string | null | undefined): string {
  switch (origen) {
    case "formulario": return "bg-blue-50 text-blue-600";
    case "feria": return "bg-purple-50 text-purple-600";
              ...
              >
                <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-[#1A4A3A]">{stage.label}</span>
                          ...
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`bg-gray-50 border border-gray-200 rounded-md p-3 cursor-grab active:cursor-grabbing hover:border-[#2D6A4F]/30 transition-colors ${
                                snapshot.isDragging ? "shadow-lg border-[#2D6A4F]/40" : ""
                              }`}
                            >
                              <div className="text-sm font-medium text-[#1A1A1A]">{lead.nombre}</div>
                              {lead.empresa && (
                                <div className="text-xs text-gray-500 mt-0.5">{lead.empresa}</div>
                              )}
                              <div className="flex items-center justify-between mt-2">
                                <span className={`text-xs px-2 py-0.5 rounded-md ${origenBadgeClass(lead.origen)}`}>
                                  {lead.origen || "manual"}
                                </span>
                                {lead.ultimoContacto && (
        ...
        <div className="fixed inset-0 bg-black/30 z-40 flex items-end sm:items-center justify-center">
          <div className="bg-white rounded-t-xl sm:rounded-md shadow-lg w-full sm:max-w-md overflow-hidden max-h-[92vh] flex flex-col">
            ...
            <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1">
              <div>
                ...
                <input type="text" required value={form.nombre}
                  ...
                  className={inputClass} placeholder="Nombre del contacto" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Empresa</label>
                <input type="text" value={form.empresa}
                  onChange={(e) => setForm((f) => ({ ...f, empresa: e.target.value }))}
                  className={inputClass} placeholder="Nombre de la empresa" />
              </div>
              <div>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
              <button onClick={closeModal} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">
                Cancelar
              </button>
