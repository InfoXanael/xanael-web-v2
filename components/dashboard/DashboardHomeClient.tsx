...
function getLast7DaysChartData(forms: ChartForm[]) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({ label: DAY_NAMES[d.getDay()], date: d.toISOString().slice(0, 10), count: 0 });
  }
  for (const form of forms) {
    const formDate = new Date(form.createdAt as string).toISOString().slice(0, 10);
...
export default function DashboardHomeClient({
  ...
  chartForms: ChartForm[];
  newFormsCount: number;
  leadsCount: number;
}) {
  const chartData = getLast7DaysChartData(chartForms);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-md p-5">
          ...
          <p className="text-2xl font-bold text-[#1A1A1A]">{newFormsCount}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-md p-5">
          <Users className="w-5 h-5 text-[#2D6A4F] mb-3" />
          <p className="text-sm text-gray-500">Leads en pipeline</p>
          <p className="text-2xl font-bold text-[#1A1A1A]">{leadsCount}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-md p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        <div className="bg-white border border-gray-200 rounded-md p-5 flex flex-col">
          <h2 className="text-lg font-semibold text-[#1A4A3A] mb-4">Formularios recibidos</h2>
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chartData}>
                ...
                <Tooltip contentStyle={{ backgroundColor: "#fff", border: "1px solid #E5E7EB", borderRadius: "6px", fontSize: "13px" }} cursor={{ fill: "#F0F4F2" }} />
                <Bar dataKey="formularios" fill="#2D6A4F" radius={[4, 4, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-md p-5 flex flex-col">
          <h2 className="text-lg font-semibold text-[#1A4A3A] mb-4">Actividad reciente</h2>
          {recentForms.length === 0 ? (
            <p className="text-sm text-gray-400">No hay formularios recientes.</p>
          ) : (
            <ul className="flex-1">
              {recentForms.map((form, i) => (
                <li key={form.id} className={`flex items-center gap-3 py-3 ${i < recentForms.length - 1 ? "border-b border-gray-100" : ""}`}>
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${ESTADO_COLORS[form.estado] || "bg-gray-300"}`} />
                  <span className="text-sm text-[#1A1A1A] flex-1 min-w-0 truncate">
                    {form.nombre} &mdash; {form.tipo}
                  </span>
                  <span className="text-xs text-gray-400 flex-shrink-0">{timeAgo(form.createdAt)}</span>
                </li>
              ))}
            </ul>
          )}
